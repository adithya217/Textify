...
void DetectText::detect() {
	...
	__android_log_print(ANDROID_LOG_DEBUG, "Debug-detect()", "before converting original image to gray");
	cvtColor(originalImage_, imGray, CV_RGB2GRAY);
	__android_log_print(ANDROID_LOG_DEBUG, "Debug-detect()", "after converting original image to gray");
...
DetectText::getBoundingBoxes(Mat& image) {
	...
	Mat imGray(originalImage_.size(), CV_8UC1, Scalar(0));
	__android_log_print(ANDROID_LOG_DEBUG, "Debug-getBoundingBoxes()", "before converting original image to gray");
	cvtColor(originalImage_, imGray, CV_RGB2GRAY);
	__android_log_print(ANDROID_LOG_DEBUG, "Debug-getBoundingBoxes()", "after converting original image to gray");
...
void DetectText::preprocess(Mat& image) {
	...
	text << "Image size" << image.rows << "x" <<image.cols << endl;
	__android_log_print(ANDROID_LOG_DEBUG, "Debug-preprocess()", text.str().c_str());
