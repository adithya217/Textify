...
extern "C" {
	...
	JNIEXPORT jlong JNICALL Java_com_vearch_textify_DetectTextNative_create(JNIEnv* env, jobject jobj) {
		DetectText* dt = new DetectText();
		return reinterpret_cast<jlong>(dt);
	}

	JNIEXPORT void JNICALL Java_com_vearch_textify_DetectTextNative_destroy(JNIEnv* env, jobject jobj, jlong detectPtr) {
		delete toDetectTextNative(detectPtr);
	}

	JNIEXPORT jintArray JNICALL Java_com_vearch_textify_DetectTextNative_getBoundingBoxes(JNIEnv* env, jobject jobj, jlong detectPtr, jlong matAddress) {
